...
@push('css')
<link href="{{asset('css/marca/marca_edit.css')}}" rel="stylesheet" />
@endpush

@section('content')
<div class="container-fluid px-4 blurred">
    <h1 class="mt-4 title"><i class="fa-solid fa-square-check icons"></i> Editar Marca</h1>
        ...
        <form action="{{ route('marcas.update',['marca'=> $marca]) }}" method="post">
            ...
            <div class="row g-4">
                ...
                <div class="col-md-12">
                    ...
                    <textarea name="descripcion" id="descripcion" row="3" class="form-control">{{old('descripcion',$marca->caracteristica->descripcion)}}</textarea>
                    @error('descripcion')
                    <small class="text-danger">{{'*'.$message}}</small>
                    @enderror
                </div>
